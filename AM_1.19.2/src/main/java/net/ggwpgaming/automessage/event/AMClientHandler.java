...
@Mod.EventBusSubscriber(modid = AutoMessage.MOD_ID, value = Dist.CLIENT)
public class AMClientHandler {
    ...
    @SubscribeEvent
    public static void clientServerTickEvent(TickEvent.ServerTickEvent event)
    {
        if (FMLEnvironment.dist == Dist.CLIENT && AMClientConfigs.MESSAGES.get().size() != 0 && event.getServer().getPlayerList().getPlayers().size() != 0) {
//            AutoMessage.println("we are ticking on a client server!");

            if (CLIENT_LIMIT_COUNTER.size() == 0 )
            {
                        ...
                        if (players.get(i).getStringUUID().equals(retrievedList.get(i).UUID.toString())) {
                            for (int messageIndex = 0; messageIndex < AMClientConfigs.MESSAGES.get().size(); messageIndex++) {
                                ...
                                if (!AMClientConfigs.MESSAGES.get().get(messageIndex).equals("") && (event.getServer().getTickCount() % (AMClientConfigs.INTERVALS.get().get(messageIndex) * 20) == 0) && event.phase == TickEvent.Phase.START) {
                                    if ((CLIENT_LIMIT_COUNTER.get(messageIndex) < AMClientConfigs.LIMITS.get().get(messageIndex) &&
                                            ...
                                            (AMClientConfigs.HARD_LIMITS.get().get(messageIndex) == 0))
                                    {
                                        ...
                                        (
                                            Component.literal(AMClientConfigs.MESSAGES.get().get(messageIndex))
                                            .withStyle(
                                                style -> style.withClickEvent( new ClickEvent( ClickEvent.Action.OPEN_URL, String.valueOf( AMClientConfigs.LINKS.get().get(thisMessageIndex) ) ) )
                                            )
                                        );
